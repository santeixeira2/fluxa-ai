...
const GUIDANCE_CONFIG = {
  raised:     { label: 'Guidance ↑', color: 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20' },
  lowered:    { label: 'Guidance ↓', color: 'text-red-400 bg-red-400/10 border-red-400/20' },
  maintained: { label: 'Guidance →', color: 'text-blue-400 bg-blue-400/10 border-blue-400/20' },
  none:       { label: '',            color: '' },
};

const TONE_CONFIG = {
  confident: { label: 'Confiante', color: 'text-emerald-300/60' },
  cautious:  { label: 'Cauteloso', color: 'text-yellow-300/60' },
...
function QuarterCard({ q }: { q: SentimentQuarter }) {
  ...
  const guidance = GUIDANCE_CONFIG[q.guidance];
  const tone = TONE_CONFIG[q.tone];
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <div className="flex items-center gap-1.5 flex-wrap justify-end">
          ...
          {guidance.label && (
            <span className={`text-[9px] font-mono tracking-widest uppercase px-1.5 py-0.5 rounded border ${guidance.color}`}>
              {guidance.label}
            </span>
          )}
        </div>
      ...
      <div className="h-[3px] rounded-full bg-white/[0.06] overflow-hidden mb-2">
        <div
          className={`h-full rounded-full transition-all duration-500 ${sentimentColor(q.sentiment)}`}
          style={{ width: `${barPct}%` }}
        />
      </div>
      ...
      {expanded && q.summary && (
        <p className="mt-3 text-[11px] leading-relaxed text-white/50 border-t border-white/[0.05] pt-3">
          {q.summary}
        </p>
      )}
    </div>
    ...
    return (
      <div className="flex items-center justify-center h-24 rounded-xl border border-white/[0.06] bg-white/[0.02]">
        <span className="text-[11px] font-mono tracking-widest text-white/30 uppercase">
          Dados não disponíveis para este ativo
        </span>
      </div>
    );
  }
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between mb-1">
        <span className="text-[10px] font-mono tracking-widest text-white/30 uppercase">
          Sentimento — Resultados Trimestrais
        </span>
      ...
      <p className="text-[9px] text-white/20 text-right mt-1">
        Fonte: SEC EDGAR · Análise via IA · Clique para expandir
      </p>
    </div>
  );
}
